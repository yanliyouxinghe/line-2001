		...
		<div id="item">
				...
				<div class="fl preview-wrap">
					...
					<div class="zoom">
						...
						<div id="preview" class="spec-preview">
							<span class="jqzoom"><img jqimg="{{$seckill->goods_img}}" src="{{$seckill->goods_img}}" width="400px" height="400px" /></span>
						</div>
						<!--下方的缩略图-->
						<div class="spec-scroll">
							...
							<div class="items">
								<ul>
									<li><img src="{{$seckill->goods_img}}" bimg="{{$seckill->goods_img}}" onmousemove="preview(this)" /></li>
					...
					<div class="product-collect">
						<a href="javascript:;" id="star"><img src="" alt=""> 收藏</a>
					</div>
				</div>
				<div class="fr itemInfo-wrap">
					<div class="sku-name">
						<h4>{{$seckill->goods_name}}</h4>
						<input type="hidden" name="goods_id" id="goods_id" value="{{$goods_id}}"> 
						<input type="hidden" name="seckill_id" id="seckill_id" value="{{$seckill->seckill_id}}"> 

					</div>
					<div class="news">
						<span><img src="/static/img/_/clock.png"/>品优秒杀</span>
						<span class="overtime"><span class="timespan"></span></span>
						</div>
					<div class="summary">
						...
						<div class="summary-wrap">
							<div class="fl title">
								<div class="control-group">
									<div class="controls">
										<input autocomplete="off" type="buuton" value="1" minnum="1" class="itxt" />
										<a href="javascript:void(0)" class="increment plus" disabled>+</a>
										<a href="javascript:void(0)" class="increment mins" disabled>-</a>
									</div>
								</div>
							</div>
							<div class="fl">
								<ul class="btn-choose unstyled">
									<li>
										<button target="_blank" class="sui-btn  btn-danger addshopcar seckill">立即结算</button>
									</li>
								</ul>
							</div>
						</div>
			...
			<script>
			  ...
			  setInterval(function () {
			    ...
			    if(day<=0&&hour<=0&&hour<=0&&seconds<=0){
			    	$('.addshopcar').addClass('disabled',true);
					$('.timespan').html("活动以结束");return;
			    }
			    if(day <= 0){
 						$('.timespan').html("距离结束："+ hour + "小时" + minute + "分钟" + seconds + "秒");
			    }else{
			    	 $('.timespan').html("距离结束："+day + "天" + hour + "小时" + minute + "分钟" + seconds + "秒");
			    }
...
<script>	
	...
	$('.seckill').click(function(){
		...
		$.ajax({
			...
			dataType:'json',
			type:'post',
			data : {goods_id:goods_id,seckill_id:seckill_id},
			success:function(retul){
			  if(retul.code==0){
				  var goods_id = retul.data.goods_id;
				  var seckill_id = retul.data.seckill_id;
				//   location.href="/address?seckill_id="+seckill_id+"&"+"goods_id="+goods_id;
